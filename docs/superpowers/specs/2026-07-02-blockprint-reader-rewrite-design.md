...
- **× 加速定义**：wall-clock 中位数（5 轮取中位），输入为 50 MB fixture（5 MB 与 200 MB 用于 sanity check，不作为门禁判定）
- 任何改动后跑同一基准，未达标不合并
...
**改动**：
- **修改** `NbtReader.readRoot(InputStream)`：当前实现是 `readRoot(input.readBytes())` 的壳；改为直接用 `DataInputStream` 流式读（与 `readRoot(ByteArray)` 路径同源，但不再委托 readBytes）。`readRoot(ByteArray)` 签名保留
- `BlockPrintReader.read(InputStream)` 改用流式 NbtReader → 分发到 `FormatDetector.detect(root)` → 对应 `*Reader.parse(root)`。gzip 检测从字节级下沉到 `PushbackInputStream` 嗅 2 字节
...
**改动**：
- 抽出 `internal/PackedBlocks.kt`，把 `unpack` 改为对 nbits=4 / 8 / 16 的特化路径（**这些是常见宽度假设**——实现前先用第一版 benchmark 统计一批 Litematica 文件的 nbits 分布，确认 4/8/16 覆盖 ≥ 90% 文件再上特化；否则改成对实际高频宽度特化）
...
测试覆盖率用项目当前配置的覆盖率工具；若项目当前未配置则用 IntelliJ IDEA 自带 runner（输出到 `build/reports/jvmTest/index.html`）。Kover 等其它工具可在实施阶段评估是否引入。
